...
const (
	joinSubcommand      = "join"
	initSubcommand      = "init"
	ocneCommandTemplate = "kubeadm %s --config /etc/kubeadm.yml %s"
)
...
func NewInitControlPlane(input *ControlPlaneInput) ([]byte, string, error) {
	...
	return render(&input.BaseUserData, input.Ignition, ocneConfig)
}

func render(input *cloudinit.BaseUserData, ignitionConfig *bootstrapv1.IgnitionSpec, ocneConfig string) ([]byte, string, error) {
	...
	return clc.Render(input, clcConfig, ocneConfig)
}
