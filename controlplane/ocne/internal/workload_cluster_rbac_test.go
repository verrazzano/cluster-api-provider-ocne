...
func TestCluster_ReconcileKubeletRBACBinding_NoError(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "creates role and role binding for Kubernetes/ocne < v1.24",
			client:  fake.NewClientBuilder().Build(),
			version: semver.MustParse("1.23.3"),
			want: &wantRBAC{
				role:        ctrlclient.ObjectKey{Namespace: metav1.NamespaceSystem, Name: "ocne:kubelet-config-1.23"},
				roleBinding: ctrlclient.ObjectKey{Namespace: metav1.NamespaceSystem, Name: "ocne:kubelet-config-1.23"},
			},
		},
		{
			name: "tolerates existing role binding for Kubernetes/ocne < v1.24",
			client: fake.NewClientBuilder().WithObjects(
				&rbacv1.RoleBinding{ObjectMeta: metav1.ObjectMeta{Namespace: metav1.NamespaceSystem, Name: "ocne:kubelet-config-1.23"}, RoleRef: rbacv1.RoleRef{
					Name: "ocne:kubelet-config-1.23",
				}},
				&rbacv1.Role{ObjectMeta: metav1.ObjectMeta{Namespace: metav1.NamespaceSystem, Name: "ocne:kubelet-config-1.23"}, Rules: []rbacv1.PolicyRule{{
					Verbs:         []string{"get"},
					APIGroups:     []string{""},
		...
		},
		{
			...
			want: &wantRBAC{
				role:        ctrlclient.ObjectKey{Namespace: metav1.NamespaceSystem, Name: "ocne:kubelet-config"},
				roleBinding: ctrlclient.ObjectKey{Namespace: metav1.NamespaceSystem, Name: "ocne:kubelet-config"},
			},
		},
		{
			name:    "creates role and role binding for Kubernetes/ocne >= v1.24 ignoring pre-release and build tags",
			client:  fake.NewClientBuilder().Build(),
		...
		},
		{
			name: "tolerates existing role binding for Kubernetes/ocne >= v1.24",
			client: fake.NewClientBuilder().WithObjects(
				&rbacv1.RoleBinding{ObjectMeta: metav1.ObjectMeta{Namespace: metav1.NamespaceSystem, Name: "ocne:kubelet-config"}, RoleRef: rbacv1.RoleRef{
					Name: "ocne:kubelet-config",
				}},
				&rbacv1.Role{ObjectMeta: metav1.ObjectMeta{Namespace: metav1.NamespaceSystem, Name: "ocne:kubelet-config"}, Rules: []rbacv1.PolicyRule{{
					Verbs:         []string{"get"},
					APIGroups:     []string{""},
					Resources:     []string{"configmaps"},
