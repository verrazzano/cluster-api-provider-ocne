...
import (
	...
	containerutil "github.com/verrazzano/cluster-api-provider-ocne/util/container"
...
var (
	// Starting from v1.22.0 ocne dropped the usage of the ClusterStatus entry from the ocne-config ConfigMap
	// so we're not anymore required to remove API endpoints for control plane nodes after deletion.
	//
	// NOTE: The following assumes that ocne version equals to Kubernetes version.
	minKubernetesVersionWithoutClusterStatus = semver.MustParse("1.22.0")

	// Starting from v1.21.0 ocne defaults to systemdCGroup driver, as well as images built with ImageBuilder,
	// so it is necessary to mutate the kubelet-config-xx ConfigMap.
	//
	// NOTE: The following assumes that ocne version equals to Kubernetes version.
	minVerKubeletSystemdDriver = semver.MustParse("1.21.0")

	// Starting from v1.24.0 ocne uses "kubelet-config" a ConfigMap name for KubeletConfiguration,
...
func (w *Workload) UpdateKubeletConfigMap(ctx context.Context, version semver.Version) error {
	...
	if version.GE(minVerKubeletSystemdDriver) {
		...
		// If the value is not already explicitly set by the user, change according to ocne/image builder new requirements.
		if cgroupDriver == "" {
...
func (w *Workload) updateClusterStatus(ctx context.Context, mutator func(status *bootstrapv1.ClusterStatus), version semver.Version) error {
	return retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		key := ctrlclient.ObjectKey{Name: ocneConfigKey, Namespace: metav1.NamespaceSystem}
		configMap, err := w.getConfigMap(ctx, key)
		if err != nil {
			return errors.Wrap(err, "failed to get ocneConfigMap")
		}

		currentData, ok := configMap.Data[clusterStatusKey]
		if !ok {
			return errors.Errorf("unable to find %q in the ocne-config ConfigMap", clusterStatusKey)
		}

		currentClusterStatus, err := ocnetypes.UnmarshalClusterStatus(currentData)
		if err != nil {
			return errors.Wrapf(err, "unable to decode %q in the ocne-config ConfigMap's from YAML", clusterStatusKey)
		}
		...
		if !reflect.DeepEqual(currentClusterStatus, updatedClusterStatus) {
			updatedData, err := ocnetypes.MarshalClusterStatusForVersion(updatedClusterStatus, version)
			if err != nil {
				return errors.Wrapf(err, "unable to encode %q ocne-config ConfigMap's to YAML", clusterStatusKey)
			}
			configMap.Data[clusterStatusKey] = updatedData
			if err := w.Client.Update(ctx, configMap); err != nil {
				return errors.Wrap(err, "failed to upgrade the ocneConfigMap")
			}
		}
		return nil
	})
...
func (w *Workload) updateClusterConfiguration(ctx context.Context, mutator func(*bootstrapv1.ClusterConfiguration), version semver.Version) error {
	return retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		...
		if !ok {
			return errors.Errorf("unable to find %q in the ocne-config ConfigMap", clusterConfigurationKey)
		}

		currentObj, err := ocnetypes.UnmarshalClusterConfiguration(currentData)
		if err != nil {
			return errors.Wrapf(err, "unable to decode %q in the ocne-config ConfigMap's from YAML", clusterConfigurationKey)
		}
		...
		if !reflect.DeepEqual(currentObj, updatedObj) {
			updatedData, err := ocnetypes.MarshalClusterConfigurationForVersion(updatedObj, version)
			if err != nil {
				return errors.Wrapf(err, "unable to encode %q ocne-config ConfigMap's to YAML", clusterConfigurationKey)
			}
...
func (w *Workload) GetAPIServerCertificateExpiry(ctx context.Context, ocneConfig *bootstrapv1.OCNEConfig, nodeName string) (*time.Time, error) {
	...
	p := proxy.Proxy{
		...
		Port:       int(calculateAPIServerPort(ocneConfig)),
	}
	...
	// Return the expiry of the peer certificate with cn=kube-apiserver (which is the one generated by ocne).
	var kubeAPIServerCert *x509.Certificate
	for _, cert := range conn.ConnectionState().PeerCertificates {
		if cert.Subject.CommonName == ocneAPIServerCertCommonName {
			kubeAPIServerCert = cert
		}
	}
	if kubeAPIServerCert == nil {
		return nil, errors.Wrapf(err, "unable to get certificate expiry for kube-apiserver on Node/%s: couldn't get peer certificate with cn=%q", nodeName, ocneAPIServerCertCommonName)
	}
...
//   - else either k8s.gcr.io or registry.k8s.io depending on the default registry of the ocne
...
// tl;dr is that the imageRepository must be in sync with the default registry of ocne.
// Otherwise ocne preflight checks will fail because ocne is trying to pull the CoreDNS image
...
func ImageRepositoryFromClusterConfig(clusterConfig *bootstrapv1.ClusterConfiguration, kubernetesVersion semver.Version) string {
	...
	// corresponding ocne binary.
	if kubernetesVersion.GTE(ocne.MinKubernetesVersionImageRegistryMigration) &&
		kubernetesVersion.LT(ocne.NextKubernetesVersionImageRegistryMigration) {
		return ocne.GetDefaultRegistry(kubernetesVersion)
	}
