...
func initWebhookInstallOptions() envtest.WebhookInstallOptions {
	...
	bootstrapyamlFile, err := os.ReadFile(filepath.Join(root, "bootstrap", "ocne", "config", "webhook", "manifests.yaml")) //nolint:gosec
	if err != nil {
	...
	controlplaneyamlFile, err := os.ReadFile(filepath.Join(root, "controlplane", "ocne", "config", "webhook", "manifests.yaml")) //nolint:Gosec
	if err != nil {
